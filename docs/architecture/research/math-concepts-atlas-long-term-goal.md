...
- `math-paper-exemplars.html`: created a full-depth worked-exemplar layer applying the nine-slot protocol to RAGEN-2, ATLAS, Rare Event Analysis/Tail Risks, Midtraining Bridges, Quotient-Space Diffusion, Alignment-Sensitive Minimax Rates, and Distributional Equivalence, showing how each paper's problem, state space, mathematical object, invariant, allowed movement, working condition, evidence check, failure boundary, and cross-paper role connect into the atlas.
